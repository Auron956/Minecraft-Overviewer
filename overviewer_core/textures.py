...
# flowers
@material(blockid=38, data=list(range(13)), transparent=True, solid=False)
def flower(self, blockid, data):
    flower_map = ["poppy", "blue_orchid", "allium", "azure_bluet", "red_tulip", "orange_tulip",
                  "white_tulip", "pink_tulip", "oxeye_daisy", "dandelion", "cornflower",
                  "wither_rose", "lily_of_the_valley"]
...
def wire(self, blockid, data):
    ...
    redstone_wire_t = self.load_image_texture("assets/minecraft/textures/block/redstone_dust_line0.png")
...
def lectern(self, blockid, data):
    ...
    def create_tile(img_src, coord_crop, coord_paste, rot):
        # Takes an image, crops a region, the finally pastes it
        #   onto a 16x16 image
        img_out = Image.new("RGBA", (16, 16), self.bgcolor)
        img_in = img_src.crop(coord_crop)
        if rot != 0: img_in = img_in.rotate(rot, expand=True)
        alpha_over(img_out, img_in, coord_paste, img_in)
        return img_out
    ...
    if (data & 0b11) in [0, 2]: img_stand = img_stand.transpose(Image.FLIP_LEFT_RIGHT)
...
@material(blockid=11368, data=list(range(4)), transparent=True, solid=True)
def stonecutter(self, blockid, data):
    ...
    # Add saw blade
    if data in [0, 2]: blade_t = blade_t.transpose(Image.FLIP_LEFT_RIGHT)
...
@material(blockid=11369, data=list(range(12)), transparent=True, solid=True)
def grindstone(self, blockid, data):
    ...
    # This function takes small texture parts, optionally scales
    #   the texture, then pastes them onto a 16x16 image
    #   (for use with transform_image_side/top)
    def create_tile(img_src, coord, scale):
        img_out = Image.new("RGBA", (16, 16), self.bgcolor)
        if scale >= 0 and scale != 1:
            w, h = img_src.size
            img_src = img_src.resize((int(w * scale), int(h * scale)), Image.NEAREST)
        alpha_over(img_out, img_src, coord, img_src)
        return img_out
    ...
    scale_factor = 2
    side_t = create_tile(side_raw_t.crop((0, 0, 12, 12)), (2, 0), 1)
    round_ud_t = create_tile(round_raw_t.crop((0, 0, 8, 12)), (4, 2), 1)
    round_lr_t = create_tile(round_raw_t.crop((0, 0, 8, 12)), (4, 0), 1)
    pivot_outer_t = create_tile(pivot_raw_t.crop((0, 0, 6, 6)), (2, 2), scale_factor)
    pivot_lr_t = create_tile(pivot_raw_t.crop((6, 0, 8, 6)), (2, 2), scale_factor)
    pivot_ud_t = create_tile(pivot_raw_t.crop((8, 0, 10, 6)), (2, 2), scale_factor)
    leg_outer_t = create_tile(leg_raw_t.crop((6, 9, 10, 16)), (2, 2), scale_factor).rotate(rot_leg)
    leg_lr_t = create_tile(leg_raw_t.crop((12, 9, 14, 16)), (2, 2), scale_factor).rotate(rot_leg)
    leg_ud_t = create_tile(leg_raw_t.crop((2, 6, 4, 10)), (2, 2), scale_factor)
...
def door(self, blockid, data):
    ...
    door_half = "top" if data & 0x8 else "bottom"

    raw_door = self.load_image_texture("assets/minecraft/textures/block/%s_%s.png" % \
                                       (texture_map[blockid], door_half))
    
    # if you want to render all doors as closed, then force
    # closed to be True
    closed = not bool(data & 0x4)
    ...
    img = Image.new("RGBA", (24, 24), self.bgcolor)
    if (data & 0x03) == 0: # facing east when closed
        if hinge_on_left:
            if closed:
    ...
    if (data & 0x03) == 1: # facing south when closed
        if hinge_on_left:
            if closed:
                tex = self.transform_image_side(raw_door)
                tex = tex.transpose(Image.FLIP_LEFT_RIGHT)
    ...
    if (data & 0x03) == 2: # facing west when closed
        if hinge_on_left:
            if closed:
                # flip first to set the doorknob on the correct side
                tex = self.transform_image_side(raw_door.transpose(Image.FLIP_LEFT_RIGHT))
    ...
    if (data & 0x03) == 3: # facing north when closed
        if hinge_on_left:
            if closed:
                # flip first to set the doorknob on the correct side
                tex = self.transform_image_side(raw_door.transpose(Image.FLIP_LEFT_RIGHT))
...
def fence(self, blockid, data):
    ...
    fence_top = self.load_image_texture("assets/minecraft/textures/block/%s.png" % texture_map[blockid]).copy()
...
# vines
@material(blockid=106, data=list(range(32)), transparent=True)
def vines(self, blockid, data):
    ...
    side_north = raw_texture if data & 0b00100 else None # South
...
# All walls, chorus plants
@material(blockid=[199] + list(range(21000, 21013 + 1)), data=list(range(16)), transparent=True, nospawn=True)
def walls(self, blockid, data):
    # Re-arrange the bits in data based on self.rotation
    # rotation  bit: 4321
    #        0       ESWN
    #        1       NESW
...
# observer
@material(blockid=218, data=list(range(14)), solid=True, nospawn=True)
def observer(self, blockid, data):
    # Return immediately if invalid facing data
    if (data & 7) > 5: return None
    ...
    back = self.load_image_texture("assets/minecraft/textures/block/%s.png" % file_name_back).copy()
...
# shulker box
@material(blockid=219, data=list(range(134)), solid=True, nospawn=True)
def shulker_box(self, blockid, data):
    ...
    if self.rotation in [1, 2, 3] and (data & 0b111) in [2, 3, 4, 5]:
        rotation_map = {1: {2: 5, 3: 4, 4: 2, 5: 3},
                        2: {2: 3, 3: 2, 4: 5, 5: 4},
                        3: {2: 4, 3: 5, 4: 3, 5: 2}}
        data = (data & 0b11111000) | rotation_map[self.rotation][data & 0b111] 
    
    if 0 <= (data >> 3) <= 15:
        # Colored shulker box
        file_name = "shulker_%s.png" % color_map[data >> 3]
    else:
...
def jigsaw_block(self, blockid, data):
    ...
    if data == 0: # down
        img = self.build_full_block(bottom.rotate(self.rotation * 90), None, None, side.rotate(180), side.rotate(180))
    elif data == 1: # up
        ...
        img = self.build_full_block(side.rotate(270), None, None, bottom.rotate(180), side.rotate(90))
...
def glazed_terracotta(self, blockid, data):
    ...
    texture = self.load_image_texture("assets/minecraft/textures/block/%s_glazed_terracotta.png" % color_map[blockid - 235]).copy()
    ...
    if data == 0:   # South
        ...
        return self.build_full_block(texture.rotate(270), None, None, texture.rotate(90), texture_side4.rotate(180))
    elif data == 2: # North
        return self.build_full_block(texture.rotate(180), None, None, texture.rotate(180), texture_side4.rotate(90))
    elif data == 3: # East
        return self.build_full_block(texture.rotate(90), None, None, texture.rotate(270), texture_side4)
...
# sweet berry bush
@material(blockid=11419, data=list(range(4)), solid=False, transparent=True)
def sweet_berry_bush(self, blockid, data):
    img = self.load_image_texture("assets/minecraft/textures/block/sweet_berry_bush_stage%s.png" % data)
...
# Campfire
@material(blockid=11505, data=list(range(8)), solid=True, transparent=True, nospawn=True)
...
# Bell
@material(blockid=11506, data=list(range(16)), solid=True, transparent=True, nospawn=True)
