...
# flowers
@material(blockid=38, data=list(range(13)), transparent=True, solid=False, nospawn=True)
...
def door(self, blockid, data):
    ...
    door_half = "top" if data & 0x08 else "bottom"
...
# All fences
@material(blockid=[85, 188, 189, 190, 191, 192, 113], data=list(range(16)),
...
def fence(self, blockid, data):
    ...
    fence_top = self.load_image_texture("assets/minecraft/textures/block/%s.png" %
                                        texture_map[blockid]).copy()
    fence_side = fence_top.copy()
    fence_small_side = fence_top.copy()

    def drawrect(img, crop, outline=(0, 0, 0, 0), fill=(0, 0, 0, 0)):
        ImageDraw.Draw(img).rectangle(crop, outline=outline, fill=fill)
...
def stained_panes(self, blockid, data):
    t = self.load_image_texture("assets/minecraft/textures/block/%s_stained_glass.png"
                                % color_map[data & 0xf])
...
@material(blockid=[199] + list(range(21000, 21014)), data=list(range(16)),
          transparent=True, nospawn=True)
...
# shulker box
@material(blockid=219, data=[x for x in range(134) if x & 7 <= 5], solid=True, nospawn=True)
